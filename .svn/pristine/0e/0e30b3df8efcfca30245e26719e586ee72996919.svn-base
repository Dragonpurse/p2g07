...
public class Leertraject {
	...
	public void linkOnderdeel(Onderdeel onderdeel) {
		...
		for(Onderdeel o : onderdelenLijst) {
			if(onderdeel.getID() == o.getID()) {
				JOptionPane.showMessageDialog(null, "Dit onderdeel zit al in het traject.");
				return;
			}
		}
	...
	public void setOnderdelenLijst(List<Onderdeel> onderdelenLijst) {
		...
		while(it.hasNext()){
			Onderdeel o = it.next();
			if(this.getBeschikbaarVan() == null)
				o.setBeschikbaarVan(getBeschikbaarVan());
			if(this.getBeschikbaarTot() == null)
