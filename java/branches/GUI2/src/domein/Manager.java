...
public class Manager {
	...
	public boolean logIn(String email, String paswoord){
		Boolean ingelogd = ((MedewerkerMapper)medewerkerMapper).logIn(email, paswoord);
		if(ingelogd){
			setIngelogdeMedewerker(medewerkerMap.get(email));
	...
	public boolean bestaatTrajectcode(String trajectcode) {
		boolean bestaatCode = false;
		for(Leertraject lt : trajectLijst) {
			if(lt.getLeertrajectCode().equalsIgnoreCase(trajectcode))
				bestaatCode = true;
		}
		return bestaatCode;
	}
	...
	public boolean bestaatMedewerker(String email) {
		return medewerkerMap.containsKey(email);
	}
	...
	public void voegExterneMedewerkerToe(Medewerker medewerker) {
		medewerkerMapper.voegItemToe(medewerker);
		medewerkerMap.put(medewerker.getLogin(), medewerker);
	}
	
	public Medewerker getMedewerker(String key) {
		return medewerkerMap.get(key);
	}
