...
public class LeertrajectMapper extends Mapper<Leertraject> {
	...
	public List<Leertraject> geefLijstPerItem(String actorID, Map<String, Medewerker> medewerkerMap) {
		...
		dbCon.openConnection();
		try {
			String sqlQuery = "SELECT * FROM tblLeertraject WHERE Email = '"+actorID+"'";
			Connection con = dbCon.getConn();
			PreparedStatement pstmt = con.prepareStatement(sqlQuery);
		    ResultSet rs = pstmt.executeQuery();
	...
	public void voegItemToe(Leertraject lt){
		dbCon.openConnection();
		try { 
			String sqlQuery = "INSERT INTO tblLeertraject(LeertrajectCode,Titel,Omschrijving,StartDatum,Doelgroep,BeschikbaarheidVan,BeschikbaarHeidTot) VALUES ('"
	...
	private void linkMedewerkersTraject(String LeertrajectCode, Map<String, Medewerker> medewerkerMap){
		try {
			for(Medewerker m : medewerkerMap.values()) {
				String sqlQuery = "INSERT INTO tblLeertrajectMedewerker VALUES ('"
	...
	private Map<String, Medewerker> geefMedewerkersTraject(String LeertrajectCode, Map<String, Medewerker> medewerkerMap){
		...
		try {
			String sqlQuery = "SELECT * FROM tblMedewerker tg JOIN tblLeertrajectMedewerker tlg ON tlg.MedewerkerID = tg.MedewerkerID WHERE tlg.LeertrajectCode ='"+LeertrajectCode+"'";
			Connection con = dbCon.getConn();
			PreparedStatement pstmt = con.prepareStatement(sqlQuery);
			ResultSet rs = pstmt.executeQuery();
			
			while(rs.next()){
				String gebruikerID = rs.getString("Email");
